...
class ImageMetaData:
    ...
    def get_date_taken(image):
        """
            returns the date of the image captured
        Args:
            image
        Returns:
            datetime
    ...
    @staticmethod
    def get_camera_tag(image):
        """
            returns the camera which was used to capture the thermal image
        Returns:
            camera name
        """
        with exiftool.ExifToolHelper() as et:
    ...
    def is_thermal(image):
        """
            checks if an image is thermal image or not
        Returns:
            bool
        """
        with exiftool.ExifToolHelper() as et:
...
class FlirMetaDataExtractor:
    ...
    def make_data_sheet(self, images_data):
        ...
        try:
            ...
            for each_image in images_data:
                # check if the image is IR
                if "IR" in each_image["FileName"]:
    ...
    def extract_pixels(self):
        """
            extracts temperature imformation of a thermal image per pixel
        """
        fir = flirimageextractor.FlirImageExtractor()
        # fir.process_image('FLIR_20240923_073441_807.jpg')
        fir.process_image('IR_6180.jpg')
        print(fir.get_thermal_np())
...
if __name__ == "__main__":
    ...
    metadata_extractor.extract_pixels()
